...
public class MembersController {
    ...
    public DataVO getUpdateMember(@RequestBody MembersVO mvo){
        ...
        try{
            // 1) SecurityContext에서 userId 추출
            String userId = (String)SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            // [LEARN] 회원정보 수정 시도 — userId는 토큰에서 꺼냄 (클라이언트 입력 신뢰 X)
            log.info("[LEARN] 회원정보 수정 시도: m_id={}", userId);
            ...
            // [LEARN] 수정 성공 감사 로그
            log.info("[LEARN] 회원정보 수정 성공: m_id={}", userId);
        } catch (Exception e) {
            // [LEARN] catch 블록 = 예측 못 한 예외. error 레벨 + e 같이 박아야 스택트레이스 보임
            log.error("[LEARN] 회원정보 수정 중 예외", e);
            dataVO.setSuccess(Boolean.FALSE);
            dataVO.setMessage("서버 오류 : " + e.getMessage());
        }
        return dataVO;
    ...
    public DataVO getDelAccount(){
        ...
        try{
            // 1) SecurityContext에서 userId 추출
            String userId = (String)SecurityContextHolder.getContext().getAuthentication().getPrincipal();
            // [LEARN] 회원 탈퇴 시도 — 인증 무효화 먼저 해야 탈퇴 도중 재접근 차단
            log.info("[LEARN] 회원 탈퇴 시도: userId={}", userId);
            ...
            // [LEARN] 탈퇴 감사 로그
            log.info("[LEARN] 회원 탈퇴 성공: userId={}", userId);
        } catch (Exception e) {
            // [LEARN] catch 블록 = 예측 못 한 예외. error 레벨 + e 같이 박아야 스택트레이스 보임
            log.error("[LEARN] 회원 탈퇴 처리 중 예외", e);
            dataVO.setSuccess(Boolean.FALSE);
            dataVO.setMessage("서버 오류 : " + e.getMessage());
        }
        return dataVO;
