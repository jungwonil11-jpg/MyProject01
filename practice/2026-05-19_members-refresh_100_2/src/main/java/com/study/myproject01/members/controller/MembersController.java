    ...
    public DataVO getRefreshToken(@RequestBody Map<String, String> body){
        ...
        try {
            ...
            if(storeToken == null){
                ...
                log.warn("DB에 없는 토큰으로 시도");
                dataVO.setSuccess(Boolean.FALSE);
                dataVO.setMessage("잘못된 접근");
                return  dataVO;
            }
            ...
            RefreshTokenVO newRefreshToken = new RefreshTokenVO();
            newRefreshToken.setRt_user_id(userId);
            newRefreshToken.setRt_token(refreshToken);
            ...
            dataVO.setSuccess(Boolean.TRUE);
            dataVO.setMessage("새 토큰 발행");
            dataVO.setData(map);

            // [LEARN] refresh 성공 = 토큰 로테이션 완료. 어떤 사용자가 갱신했는지 감사 로그.
            log.info("refresh 성공", _____);
            ...
            log.info("만료", _____);
            dataVO.setSuccess(Boolean.FALSE);
            dataVO.setMessage("만료");
        } catch (Exception e) {
            // [LEARN] 예측 못 한 예외 — error 레벨 + 예외 객체로 스택트레이스 보존
            log.error("예외발생", _____);
            dataVO.setSuccess(Boolean.FALSE);
            dataVO.setMessage("예외발생");
        }
        return  dataVO;
    }
