...
public class GuestBookController {
    ...
    public DataVO getGuestBookUpdate(@RequestBody GuestBookVO gvo){
        ...
        log.info("게시글 수정 시도: g_idx={}, writer={}", gvo.getG_idx(), gvo.getG_writer());
        try{
            ...
            if(result == 0){
                ...
                log.warn("게시글 수정 실패: g_idx={}, writer={}", gvo.getG_idx(), gvo.getG_writer());
                dataVO.setSuccess(Boolean.FALSE);
                ...
                // [로그] 수정 성공 감사 로그
                log.info("게시글 수정 성공: g_idx={}", gvo.getG_idx());
            }
        }catch (Exception e){
            // [로그] catch 블록 = 예측 못 한 예외. error 레벨 + e 같이 박아야 스택트레이스 보임
            log.error("게시글 수정 중 예외: g_idx={}", gvo.getG_idx(), e);
            dataVO.setSuccess(Boolean.FALSE);
            dataVO.setMessage(e.getMessage());
        }
        return dataVO;
