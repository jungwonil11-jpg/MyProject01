...
public class MembersController {
    ...
    public DataVO getMypage(){
        ...
        try{
            ...
            // 2) 회원 정보 DB 조회
            MembersVO membersVO = membersService.findById(userId);
            // 3) 결과에 따라 성공/실패 응답 세팅
            if(membersVO == null){
                // [로그] 토큰은 유효한데 DB에 사용자 없음 = 계정 삭제됐는데 토큰 살아있음 = 이상 신호 (보안 알람)
                log.warn("사용자 없음. ID : {}", userId);
                dataVO.setSuccess(Boolean.FALSE);
                dataVO.setMessage("계정이 올바르지 않습니다");
            }else{
                dataVO.setSuccess(Boolean.TRUE);
                dataVO.setMessage("사용자 인증 완료됨");
    ...
    public  DataVO getUpdate(@RequestBody MembersVO mvo){
        ...
        try {
            ...
            log.warn("회원정보 수정 시도 ID : {}", userId);
            // 2) VO에 userId 세팅 (클라이언트가 id 안 보내도 서버가 토큰에서 직접 확인)
            dataVO.setData(userId);
            // 3) DB update
            membersService.memberUpdate(userId);
            // 4) 성공 응답 세팅
            dataVO.setSuccess(Boolean.TRUE);
            dataVO.setMessage("수정 성공");
            // [로그] 수정 성공 감사 로그
            log.info("수정 성공 ID : {}", userId);
        } catch (Exception e) {
    ...
    public  DataVO getDelete(){
        ...
        try {
            ...
            log.warn("회원 탈퇴 시도 ID : {}", userId);
            // 2) refreshToken 삭제 (인증 무효화 먼저 — 탈퇴 도중 재접근 차단)
            RefreshTokenVO.deleteRefreshToken(userId);
            // 3) 회원 soft delete (m_active=1, 실제 row 삭제 X)
            MembersVO.deleteMember(userId);
            // 4) 성공 응답 세팅
            dataVO.setSuccess(Boolean.TRUE);
            dataVO.setMessage("삭제 성공");
            // [로그] 탈퇴 감사 로그
            log.info("탈퇴 아이디 : {}", userId);
        } catch (Exception e) {
            // [로그] catch 블록 = 예측 못 한 예외. error 레벨 + e 같이 박아야 스택트레이스 보임
            log.error("예외 에러 : {}", e);
            dataVO.setSuccess(Boolean.FALSE);
